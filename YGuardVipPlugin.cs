...
public class YGuardVipPlugin : BasePlugin, IPluginConfig<YGuardVipConfig>
{
    ...
    private void ClearTrackedMenu(CCSPlayerController player)
    {
        ...
        _menuSilentUntil[player.Slot] = DateTime.UtcNow.AddSeconds(15);
        try { MenuManager.CloseActiveMenu(player); } catch { /* ignore */ }
    }
    ...
    /// <summary>Hide from everyone — does not need a resolved player (!1 always matches).</summary>
    private static bool ShouldAlwaysHideFromChat(string raw)
    {
        if (string.IsNullOrEmpty(raw)) return false;

        // !1 !9 /3 .2 — always private (menu picks)
        if (raw.Length <= 3 && raw[0] is '!' or '/' or '.')
        {
            var num = raw[1..].Trim();
            if (int.TryParse(num, out var n) && n is >= 1 and <= 9)
                return true;
        }
    ...
    {
        if (ShouldAlwaysHideFromChat(raw))
            return true;

        if (string.IsNullOrEmpty(raw)) return false;

        // Bare "1".."9" only while panel silence is active
        if (int.TryParse(raw, out var choice) && choice is >= 1 and <= 9 && raw.Length == 1
    ...
    private bool TryHandleSilentVipText(CCSPlayerController player, string raw)
    {
        ...
        if (int.TryParse(maybeNum, out var choice) && choice is >= 1 and <= 9
            && raw.Length <= 3
    ...
    private static CCSPlayerController? ResolveChatPlayer(int eventUserId)
    {
        foreach (var p in Utilities.GetPlayers())
        {
            if (!p.IsValid) continue;
            if (p.UserId == eventUserId) return p;
            // Some builds put slot/index in player_chat userid
            if (p.Slot == eventUserId) return p;
            try { if ((int)p.Index == eventUserId) return p; } catch { /* ignore */ }
        }
    ...
    private HookResult OnPlayerChatHide(EventPlayerChat @event, GameEventInfo info)
    {
        try
        {
            ...
            var player = ResolveChatPlayer(@event.Userid);

            // Hide first (even if player resolve fails) — fixes !1 still leaking
            if (ShouldAlwaysHideFromChat(raw)
                || (player != null && IsSilentVipText(player, raw)))
            {
            ...
            if (player != null && player.IsValid && !_silentChatHandled.Contains(player.SteamID))
    ...
    private void OpenVoteKickMenu(CCSPlayerController starter)
    {
        ...
        foreach (var p in Utilities.GetPlayers().OrderBy(x => x.PlayerName))
        {
            ...
            any = true;
            var name = SafeName(p.PlayerName);
            var sid = p.SteamID;
    ...
    private void StartVoteKick(CCSPlayerController starter, ulong targetSteam, string targetName)
    {
        try
        {
            if (!IsPlayerOk(starter) || !IsVip(starter) || _voteActive)
                return;
            ...
            var target = Utilities.GetPlayers().FirstOrDefault(p => IsPlayerOk(p) && p.SteamID == targetSteam);
            if (target == null)
            {
            ...
            targetName = SafeName(target.PlayerName);
            _voteActive = true;
            ...
            Broadcast($"{ChatColors.Grey}By {SafeName(starter.PlayerName)} · need {ChatColors.Lime}{needed} YES {ChatColors.Grey}· {Config.VoteKickDurationSeconds}s");
            Broadcast($"{ChatColors.Lime}Type !yes {ChatColors.Grey}or {ChatColors.Red}!no");

            // No CenterHtml ballots / no PrintToCenterHtml HUD — those crashed on vote
            _voteTimer?.Kill();
            _voteTimer = AddTimer(Config.VoteKickDurationSeconds, () =>
            {
                try { FinishVoteKick(); }
                catch (Exception ex) { Logger.LogError(ex, "Vote timer finish failed"); }
            });
    ...
    private void CastVote(CCSPlayerController player, bool yes)
    {
        try
        {
            ...
            // Defer finish — finishing inside menu/chat callback crashes some CSS builds
            if (_voteYes.Count >= VotesNeeded())
            {
                AddTimer(0.15f, () =>
                {
                    try { FinishVoteKick(); }
                    catch (Exception ex) { Logger.LogError(ex, "Deferred FinishVoteKick failed"); }
                });
            }
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "CastVote failed");
        }
    }
    ...
    private int VotesNeeded()
    {
        try
        {
            var voters = Utilities.GetPlayers().Count(p =>
                IsPlayerOk(p) && p.SteamID != _voteTargetSteam);
    ...
    private void FinishVoteKick()
    {
        ...
        try
        {
            ...
            var targetSteam = _voteTargetSteam;
            var targetName = _voteTargetName;

            // Clear state BEFORE kick so re-entry is safe
            _voteActive = false;
            _voteYes.Clear();
            _voteNo.Clear();
            _voteTargetSteam = 0;
            ...
            if (passed)
            {
                Broadcast($"{ChatColors.Lime}Vote kick PASSED ({yes}/{needed}) — kicking {targetName}");
                Server.NextFrame(() =>
                {
                    try
                    {
                        var target = Utilities.GetPlayers()
                            .FirstOrDefault(p => IsPlayerOk(p) && p.SteamID == targetSteam);
                        if (target == null) return;
                        var uid = target.UserId;
                        if (uid.HasValue)
                            Server.ExecuteCommand($"kickid {uid.Value} Vote kicked");
                    }
                    catch (Exception ex)
                    {
                        Logger.LogError(ex, "Vote kick execute failed");
                    }
                });
            }
            else
            {
                Broadcast($"{ChatColors.Red}Vote kick FAILED ({yes}/{needed}) — {targetName} stays");
            }
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "FinishVoteKick failed");
            _voteActive = false;
            _voteYes.Clear();
            _voteNo.Clear();
            _voteTargetSteam = 0;
    ...
    private static string SafeName(string? name)
    {
        if (string.IsNullOrWhiteSpace(name))
            return "player";
        // Strip chat color codes that can break PrintToChatAll
        var n = name;
        foreach (var c in new[] { '\x01', '\x02', '\x03', '\x04', '\x05', '\x06', '\x07', '\x08', '\x09', '\x0A', '\x0B', '\x0C' })
            n = n.Replace(c.ToString(), "");
        if (n.Length > 32) n = n[..32];
        return n;
    }
    ...
    private static CCSPlayerController? ResolveSmokeThrower(CSmokeGrenadeProjectile smoke)
    {
        try
        {
            var throwerEnt = smoke.Thrower.Value;
            if (throwerEnt == null || !throwerEnt.IsValid)
                return null;

            if (throwerEnt.Controller.Value is CCSPlayerController c1 && c1.IsValid)
                return c1;

            if (throwerEnt is CCSPlayerPawn pawn)
            {
                var orig = pawn.OriginalController.Value;
                if (orig != null && orig.IsValid)
                    return orig;
            }
        }
        catch
    ...
    private bool TryGetSmokeRgb(string colorName, out float r, out float g, out float b)
    {
        r = g = b = 0;
        foreach (var (key, rgb) in Config.SmokeColors)
        {
            if (!key.Equals(colorName, StringComparison.OrdinalIgnoreCase) || rgb.Length < 3)
                continue;
            r = rgb[0];
            g = rgb[1];
            b = rgb[2];
